...
export default function BountyDetail() {
  ...
  const bountyId = router.query.bountyId as string | undefined;
  const { currentUser } = useAuth();

  const [bounties, setBounties] = useState(MOCK_BOUNTIES);
  const [showApply, setShowApply] = useState(false);

  // router.query is empty on the first client render; wait for it.
  ...
  if (!bounty) {
    return (
      <div className="bg-surface-card border border-border rounded-xl p-12 text-center">
        ...
        <Link href="/bounties" className="text-sm text-accent hover:text-accent-hover mt-3 inline-block">
          ← Back to Bounties
        </Link>
      </div>
    );
