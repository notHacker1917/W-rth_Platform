  ...
  return (
    <article className="bg-surface-card border border-border rounded-xl overflow-hidden hover:border-surface-elevated transition-all hover:shadow-sm hover:shadow-black/40">
      <div className="flex items-start gap-3 p-4 pb-3">
        <Link href={`/profile/${author.id}`} className="shrink-0"><Avatar src={author.avatarUrl} alt={author.name} size="md" /></Link>
        <div className="flex-1 min-w-0">
          <div className="flex items-center flex-wrap gap-x-2 gap-y-0.5">
            <Link href={`/profile/${author.id}`} className="text-sm font-semibold text-text-primary hover:text-accent transition-colors truncate">{author.name}</Link>
